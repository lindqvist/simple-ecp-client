	...
	protected void marshallAttributes(XMLObject xmlObject, Element domElement)
		...
		if (request.getResponseConsumerURL() != null) {
			domElement.setAttributeNS(null,
					Request.RESPONSE_CONSUMER_URL_ATTRIB_NAME,
					request.getResponseConsumerURL());
		}
		if (request.getService() != null) {
			domElement.setAttributeNS(null, Request.SERVICE_ATTRIB_NAME,
					request.getService());
		}
		...
		if (request.isSOAP11MustUnderstandXSBoolean() != null) {
			XMLObjectSupport.marshallAttribute(
					Request.SOAP11_MUST_UNDERSTAND_ATTR_NAME, request
							.isSOAP11MustUnderstandXSBoolean().toString(),
					domElement, false);
		}
