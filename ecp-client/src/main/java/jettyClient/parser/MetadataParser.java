...
public class MetadataParser {
	 ...
	 * contain many of these, cant handle that yet)) and return the resulting
	...
	public static EntityDescriptor parseMetadata(String idpMetadataFile,
		...
		if (ValidateXML.isValidEntityDescriptor(new ByteArrayInputStream(
				entityDescriptorString.getBytes())) == false)
			return null;

		Element element = ParseHelper.extractElement(new ByteArrayInputStream(
				entityDescriptorString.getBytes()), entityDescriptorSchema);
	...
	public static IDPEntry extractEntry(String idpMetadataFile) {
		...
		String entityDescriptorSchema = "resources/schema/saml-schema-metadata-2.0.xsd"; // :(
